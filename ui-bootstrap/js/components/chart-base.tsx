...
class ChartBase extends React.Component<IChartBaseProps, null> {
    ...
    updateChart() {
        if (this.props.isVisible === false) return;

        const chartSettings = manywho.settings.global(
            'charts.' + this.props.type,
            this.props.flowKey,
            null,
        );

        const backgroundColors =
            chartSettings && chartSettings.backgroundColors
                ? chartSettings.backgroundColors
                : manywho.settings.global('charts.backgroundColors', this.props.flowKey);

        const borderColors =
            chartSettings && chartSettings.borderColors
        ...
        this.props.objectData.forEach((objectData, index) => {
            ...
            if (this.props.labels) dataset.label = this.props.labels[index];
                ...
                objectData.forEach((objectDatum, rowIndex) => {
                    this.props.columns.forEach((column, columnIndex) => {
                        ...
                        if (property)
                            switch (columnIndex) {
                                case 0:
                                    if (index === 0) data.labels.push(property.contentValue);
                                    break;
                                ...
                                case 2:
                                    {
                                        ...
                                        if (manywho.utils.isNullOrWhitespace(property.contentValue))
                                            backgroundColor =
                                                backgroundColors[
                                                    rowIndex % backgroundColors.length
                                                ];
                                ...
                                case 3:
                                    {
                                        ...
                                        if (manywho.utils.isNullOrWhitespace(property.contentValue))
                                            borderColor =
                                                borderColors[rowIndex % borderColors.length];
                    ...
                    if (
                        this.props.objectData.length > 1 &&
...
export const getChartBase = (): typeof ChartBase =>
    manywho.component.getByName(registeredComponents.CHART_BASE) || ChartBase;
