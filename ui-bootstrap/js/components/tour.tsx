...
class Tour extends React.Component<ITourProps, ITourState> {
    ...
    UNSAFE_componentWillReceiveProps(nextProps: ITourProps) {
        if (this.props.stepIndex !== nextProps.stepIndex) {
            this.setState({ foundTarget: false, style: this.state.style });
            this.domWatcher = setInterval(() => {
                this.onInterval(this.props.stepIndex);
            }, 500);
        }
    }
    ...
    componentDidUpdate(prevProps: ITourProps, prevState: ITourState) {
        if (prevState.foundTarget === false && this.state.foundTarget) {
            ...
            if (
                manywho.utils.isEqual(step.placement, 'bottom', true) ||
            ...
            if (
                manywho.utils.isEqual(step.placement, 'left', true) ||
                ...
                switch (step.align.toUpperCase()) {
                    ...
                    case 'CENTER':
                        style.top =
                            targetRect.top + targetRect.height / 2 - stepBoundingRect.height / 2;
                        break;
    ...
    render() {
        if (!this.state.foundTarget) return null;
                ...
                >
                    ...
                    {manywho.utils.isNullOrWhitespace(step.title) ? null : (
                        <div className="popover-title">
                            {step.title}
                            <button className="close" onClick={this.onDone}>
                                <span>&times;</span>
                            </button>
                        </div>
                    )}
                    ...
                    <div className="popover-content">
                        ...
                        <div className="popover-buttons">
                            {step.showBack ? (
                                <button className="btn btn-default btn-sm" onClick={this.onBack}>
                                    Back
                                </button>
                            ) : null}
                            {step.showNext ? (
                                <button className="btn btn-primary btn-sm" onClick={this.onNext}>
                                    Next
                                </button>
                            ) : null}
                        </div>
...
manywho.tours.getTargetElement = function (step: ITourStep) {
    ...
    if (step.querySelector) return document.querySelector(step.target);
