...
class TableInput extends React.Component<ITableInputProps, ITableInputState> {
    ...
    onChange = (e) => {
        if (
            ...
            manywho.utils.isEqual(this.props.contentType, manywho.component.contentTypes.date, true)
        ) {
    ...
    onBlur = () => {
        ...
        if (!this.props.isDesignTime) this.onCommit();
    };
        ...
        if (isDateTimeInput || isDateInput) {
            ...
            if (isEmptyDate) {
                ...
                const dateTime = isDateTimeInput
                    ...
                    : moment(this.state.value, ['MM/DD/YYYY', this.props.contentFormat || '']);
    ...
    render() {
        ...
        if (!this.state.isFocused) className += ' table-input-display';
        ...
        if (
            ...
            props.checked =
                this.state.value === true || manywho.utils.isEqual(this.state.value, 'true', true);
        }

        if (
            manywho.utils.isEqual(
                this.props.contentType,
                manywho.component.contentTypes.string,
                true,
            )
        ) {
            props.rows = 1;
...
export const getTableInput = (): typeof TableInput =>
    manywho.component.getByName(registeredComponents.TABLE_INPUT);
