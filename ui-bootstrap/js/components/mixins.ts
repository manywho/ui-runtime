    ...
    manywho.component.mixins.enterKeyHandler = {
        onEnter: function (e) {
            ...
            if (
                ...
                e.target.className &&
                e.target.className.indexOf('prevent-submit-on-enter') === -1 &&
                manywho.settings.global('shortcuts.progressOnEnter', this.props.flowKey, true)
            ) {
                const outcome = manywho.model
                    .getAllOutcomes(this.props.flowKey)
    ...
    manywho.component.mixins.collapse = {
        ...
        getLabel: function (label, required) {
            if (!manywho.utils.isNullOrWhitespace(label)) {
                ...
                if (required) {
                    labelContent.push(
                        React.createElement('span', { className: 'input-required' }, ' *'),
                    );
                }

                return React.createElement(
                    'h3',
                    { className: labelClasses, onClick: this.toggleVisibility },
                    labelContent,
                );
            }
